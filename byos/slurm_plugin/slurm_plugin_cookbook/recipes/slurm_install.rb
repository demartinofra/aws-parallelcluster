...
# FIXME: assuming slurm user and group already exists
# FIXME: assuming slurm dependencies already exists (packages, munge, pmix)

slurm_tarball = "#{node['byos']['local_dir']}/slurm-#{node['slurm']['version']}.tar.gz"
...
bash 'make install' do
  ...
  code <<-SLURM
    ...
    ./configure --prefix=#{node['slurm']['install_dir']} --with-pmix=/opt/pmix
